...
syn match pbPkgnameGroup /^pkgname=.*/ contains=pbIllegalPkgname,pb_k_pkgname,shSingleQuote
syn region pbMultiPkgname start=/^pkgname=(/ end=/)/ contains=pb_k_pkgname,pbValidPkgname,shSingleQuote
...
syn match pbPkgverGroup /^pkgver=.*/ contains=pbIllegalPkgver,pbValidPkgver,pb_k_pkgver,shSingleQuote
...
syn match pbPkgrelGroup /^pkgrel=.*/ contains=pbIllegalPkgrel,pbValidPkgrel,pb_k_pkgrel,shSingleQuote
...
syn match pbPkgdescGroup /^pkgdesc=.*/ contains=pbIllegalPkgdesc,pb_k_desc,pbValidPkgdesc,shSingleQuote
...
syn match pbUrlGroup /^url=.*/ contains=pbValidUrl,pb_k_url,pbIllegalUrl,shSingleQuote

" license
syn keyword pb_k_license license contained
syn keyword pbLicense PSF PerlArtistic APACHE CDDL EPL FDL GPL2 GPL3 GPL LGPL LGPL2 \.1 LGPL3 MPL PHP OPENSSL RUBY ZLIB ISC MIT PD BSD contained
syn match pbLicenseCustom /custom\(:[[:alnum:]]*\)*/ contained
syn match pbLicenseCustom /3-clause\(\s[[:alnum:]]*\)*/ contained
...
syn region pbBackupGroup start=/^backup=(/ end=/)/ contains=pb_k_backup,pbValidBackup,shSingleQuote

" arch
syn keyword pb_k_arch arch contained
syn keyword pbArch any i686 x86_64 ppc contained
...
syn region pbGroupsGroup start=/^groups=(/ end=/)/ contains=pb_k_groups,pbValidGroups,shSingleQuote

" depends
syn keyword pb_k_depends depends contained
syn match pbValidDepends /\([[:alnum:]]\|+\|-\|_\)*/ contained
syn region pbDependsGroup start=/^depends=(/ end=/)/ contains=pb_k_depends,pbValidDepends,shSingleQuote

" makedepends
syn keyword pb_k_makedepends makedepends contained
syn match pbValidMakedepends /\([[:alnum:]]\|+\|-\|_\)*/ contained
syn region pbMakedependsGroup start=/^makedepends=(/ end=/)/ contains=pb_k_makedepends,pbValidMakedepends,shSingleQuote

" optdepends
syn keyword pb_k_optdepends optdepends contained
syn match pbValidOptdepends /\([[:alnum:]]\|+\|-\|_\)*/ contained
syn region pbOptdependsGroup start=/^optdepends=(/ end=/)/ contains=pb_k_optdepends,pbValidOptdepends,shSingleQuote

" conflicts
syn keyword pb_k_conflicts conflicts contained
syn match pbValidConflicts /\([[:alnum:]]\|+\|-\|_\)*/ contained
syn region pbConflictsGroup start=/^conflicts=(/ end=/)/ contains=pb_k_conflicts,pbValidConflicts,shSingleQuote

" provides
syn keyword pb_k_provides provides contained
syn match pbValidProvides /\([[:alnum:]]\|+\|-\|_\)*/ contained
syn region pbProvidesGroup start=/^provides=(/ end=/)/ contains=pb_k_provides,pbValidProvides,shSingleQuote

" replaces
syn keyword pb_k_replaces replaces contained
syn match pbValidReplaces /\([[:alnum:]]\|+\|-\|_\)*/ contained
syn region pbReplacesGroup start=/^replaces=(/  end=/)/ contains=pb_k_replaces,pbValidReplaces,shSingleQuote
...
syn match pbInstallGroup /^install=.*/ contains=pb_k_install,pbValidInstall,pbIllegalInstall,shDeref
...
syn match pbIllegalSha256sums /[^='"()\/ ]/ contained contains=pbValidSha1sums
syn match pbValidSha256sums /[[:alnum:]]\{64\}/ contained
...
syn match pbSha256Hash /[[:alnum:]]\+/ contained contains=pbValidSha256sums
...
syn match pbOptions /\(no\)\?\(buildflags\|strip\|docs\|libtool\|emptydirs\|zipman\|ccache\|distcc\|makeflags\|force\)/ contained
...
"syn keyword pb_k_variables variables contained
"syn match pbVariable "\${[^}]*}"
"syn match pbVariable "\${[[:alpha:]]}" 
"contains=shShellVariables
"syn region pb 
"syn region pbGroupVariables matchgroup=pbValidVariables contains=shDoubleQuote
"syn region pbVariable matchgroup=shVariable start=shVariable contains=shDoubleQuote
